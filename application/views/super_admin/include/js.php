...
function validateEmail(input) {input.value = input.value.toUpperCase().replace(/[^a-zA-Z@.]/g, '');const emailPattern = /^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}$/i;
            const errorMessage = document.getElementById('error-message');if (!emailPattern.test(input.value)) {errorMessage.textContent = "Invalid email address.";
