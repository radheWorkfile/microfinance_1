<?php
    ...
    class Group_Loan_model extends CI_Model
    {
        ...
        public function get_center_member_data($id) {
            ...
            $this->db->select('mem.member_id, mem.first_name, mem.mid_name, mem.last_name, mem.mobile_no, mem.email, mem.id as mem_id, mem.referesh_disbursment_status');
        ...
        public function all_group_loan_paid_emi_data_query($where = false)
        {
            ...
            $this->db->select('gmpd.id,gmpd.member_id as gmpd_memId,gmpd.total_payble_amount, gmpd.emi_date, gmpd.paid_amount, gmpd.rest_amount, m.first_name, m.mid_name, m.last_name, m.member_id, m.center_name, mg.name as group_name')->from('group_member_payment_details as gmpd');
            $this->db->where('gmpd.branch_id', $this->session->userdata('branch_id'));
            $this->db->join('member as m', 'm.member_id = gmpd.gmpd_memId', 'left');
        ...
        public function get_group_loan_paid_emi_bill_print_data($id) {
            ...
            $this->db->select('gmpd.*, m.first_name, m.mid_name, m.last_name, m.member_id, agl.receipt_no, agl.loan_no, agl.amount, agl.tenure, agl.tenure_type, lp.loan_product_name');
            $this->db->where('gmpd.id', $id);
            $this->db->join('member as m', 'm.id = gmpd.member_id', 'left');
        ...
        public function all_refresh_disburse_loan_data_query($where = false) {
            ...
            $this->db->where(array('agl.referesh_disbursment_status' => 1));
            $this->db->join('member as m', 'm.id = agl.member_id', 'left');
            $this->db->join('master_center as mc', 'mc.id = agl.center_id', 'left');
            if (isset($where['order']) && !empty($where['order'])) {
                $this->db->order_by($field[$where['order']['0']['column']], $where['order']['0']['dir']);
