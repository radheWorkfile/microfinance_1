...
class Common_model extends CI_Model
{
    ...
    function all_data_con_or($table,$val,$or,$sel)
    {
        return $this->db->select($sel)->where($val)->or_where($or)->get($table)->result_array();
    }
    ...
    {
        return $this->db->select('mc.id,mc.center_name,mc.center_id')->where(array('mc.status' => '1','branch_id' => $this->session->userdata('branch_id')))->join('master_branch as mb','mb.id=mc.branch_id')->get('master_center as mc')->result_array();
    }
    ...
    public function test_1() 
    {
         ...
         return $this->db->select('glpd.payment_date,mc.center_id as member_id, mc.center_name as field_name, mc.status')->where(array('mc.status' => 1))->join('group_loan_payment_details as glpd','glpd.center_id=mc.id','right')->group_by('mc.center_name')->get('master_center as mc')->result_array();

        //  'glpd.payment_date'=>date('Y-m-d')
     }
